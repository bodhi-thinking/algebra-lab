...
import { valueAt } from "@/lib/challenge-types";
...
export default function AlgebraTiles() {
  ...
  // Jumps exclude the starting value, so the Lab contains inputCount + 1 values.
    ...
    <div>
      ...
      {mode === "concrete" ? (
        <div className="rounded-2xl border border-line bg-panel p-4">
          <p className="mb-2 font-mono text-[11px] tracking-wide text-ink-faint">
            constant
          </p>
          ...
          {capped && (
            <p className="mt-2 text-[11px] text-ink-faint">
              Showing the first {MAX_VISIBLE_INPUTS} values — the same
              structure continues.
            </p>
          )}
        ...
        <>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <TileButton
              label="+ x tile"
              onClick={() =>
                setCoefficient(active.id, active.coefficient + 1)
              }
            />
            <TileButton
              label="− x tile"
            ...
            <TileButton
              label="+ 1 tile"
              onClick={() =>
                setConstant(active.id, active.constant + 1)
              }
            />
            <TileButton
              label="− 1 tile"
...
function TileButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="rounded-full border border-line px-3 py-1.5 text-xs font-medium text-ink-soft hover:border-ink-soft hover:bg-line-soft hover:text-ink"
    >
      {label}
    </button>
  );
