...
import { sequenceValues } from "@/lib/challenge-types";
...
export default function Graph() {
  ...
  const { yMin, yMax } = useMemo(() => {
    ...
    visible.forEach((equation) => {
      pointsFor(equation).forEach((value) => {
  ...
  }, [visible, inputStart, inputCount]);

  const xMin = inputStart;
  const xMax = Math.max(xMin + 1, inputStart + inputCount);
  ...
  const xTicks = useMemo(
    () =>
      Array.from(
        { length: Math.min(inputCount + 1, MAX_TICKS) },
          ...
          return (
            ...
            >
              ...
              {points.map((value, index) => {
                const input = inputStart + index;
                const isSelected =
                  isActive && input === selectedInput;
                const isHovered =
                ...
                return (
                  <g key={`${equation.id}-${input}`}>
                    <circle
                      ...
                      onMouseEnter={() =>
                        setHover({ eqId: equation.id, input })
                      }
